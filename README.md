...
## 1. Previous
### 지난 미팅에서 Window size (stride)를 각각 50 (10), 75 (15), 125 (25), 150 (30), 200 (40)으로 진행한 결과를 보여드렸습니다. 이에 model input으로 R2 저항을 포함시키지 않았을 경우와 포함시켰을 경우의 RMSE와 R2를 통한 성능을 살펴보았습니다. 결과적으로 window size가 줄어들 경우 RMSE값은 감소하는 경향이 있었으며 R2 값은 큰 변화가 있지는 않았음을 확인하였습니다. 이에 극단적으로 window의 size를 줄인 결과도 살펴보았으며 해당 결과를 현 README에 업로드하였습니다. 
...
이에 window length가 20과 50 사이일 때 결과가 어떻게 나오는지 확인해보기 위해 두 가지의 경우를 추가하여 진행해보았습니다.

## 3. Final Result
### Window size를 30 (6), 40 (8) 총 두 가지 더 추가하여 결과를 살펴보면, 아래 그래프와 같이 20과 50의 경우와 거의 비슷한 결과를 보임을 확인하였습니다. 이에 데이터에 맞는 최적의 window length 및 stride의 길이를 알아볼 수 있었습니다. 이에 이전의 공기 중 취득 데이터 뿐만 아니라 물에서의 다양한 환경 변화 속에서의 데이터를 사용하여 머신러닝을 진행한다면 이에 맞는 최적의 window length를 찾아나가야 할 것 같습니다.
